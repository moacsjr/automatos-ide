...
export class SafeAutomationInterpreter {
  ...
  private async executeStep(step: AutomationStep): Promise<void> {
    ...
    switch (action) {
      case "navigate":
        if (!params?.url)
          throw new Error(`Step ${step.id}: navigate requires a url param.`);
      ...
      case "click":
        if (!params?.selector)
          throw new Error(`Step ${step.id}: click requires a selector param.`);
      ...
      case "fill":
        if (!params?.selector || !params?.value)
          throw new Error(
            `Step ${step.id}: fill requires selector and value params.`,
          );
      ...
      case "condition":
        if (!conditionConfig)
          throw new Error(`Step ${step.id}: condition config is missing.`);
      ...
      default: {
        ...
        throw new Error(
          `Action [${never}] is not supported by the interpreter.`,
        );
      }
    }
  }
